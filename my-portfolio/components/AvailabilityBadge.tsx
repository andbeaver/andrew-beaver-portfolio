...
export default function AvailabilityBadge({
  ...
  if (variant === "inline") {
    return (
      <span className="inline-flex items-center gap-2 self-start bg-emerald-50 text-emerald-700 text-xs font-semibold px-3 py-1.5 rounded-full border border-emerald-200">
        {/* Pulsing dot */}
    ...
    <div>
      ...
      <ul className="flex flex-col gap-2">
        {opportunities.map(({ label, icon }) => (
          <li
            key={label}
            className="flex items-center gap-2.5 bg-emerald-50 border border-emerald-100 rounded-lg px-3.5 py-2 text-sm text-emerald-800 font-medium"
          >
